$(document).ready(function(){
    ...
    const myFullpage = new fullpage('#fullpage', {  /* html에서 페이지 전체를 감싸는 요소 */
		...
		scrollOverflow: true, /* 컨텐츠가 넘쳐도 스크롤 금지 */
	...
	/************************************************
	 * aside quick 열고 닫기
	 * 닫혀있을 때 (open클래스가 있을 때 ) - close 클래스 교처, detail 보임
	 * 열려있을 때 (open클래스가 없을 때) - open 클래스 교체, detail 숨김
	 * 
	 */
	$('.quick .btn').on('click',function(){
		...
		if($(this).hasClass('open') == true){
			...
			$(this).addClass('open')
			$(this).attr('title','닫기')
			$('.quick .detail').slideUp(150)
		}
	})

    /*************** visual swiper ************* */
	let visual_name = ['산림복원 기금 모금','같이가치 매달기부','서울마이트리','고목나무 이야기']
